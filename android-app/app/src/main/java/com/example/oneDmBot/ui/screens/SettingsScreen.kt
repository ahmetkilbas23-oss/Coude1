...
fun SettingsScreen(modifier: Modifier = Modifier) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Section(title = "İndirme Tercihleri") {
                OutlinedTextField(
            ...
            Section(title = "1DM Bağlantısı") {
                OutlinedTextField(
                    value = pkg, onValueChange = { pkg = it; s.oneDmPackage = it },
                    label = { Text("1DM paket adı") },
                    supportingText = { Text("1DM = idm.internet.download.manager · 1DM+ = idm.internet.download.manager.plus") },
                    singleLine = true,
                    modifier = Modifier.fillMaxWidth()
                )
            }

            Section(title = "Otomatik Kontrol") {
                OutlinedTextField(
                    value = hour, onValueChange = {
                        hour = it
                        it.toIntOrNull()?.let { h -> if (h in 0..23) s.dailyCheckHour = h }
                    },
                    label = { Text("Günlük kontrol saati (0-23)") },
                    supportingText = { Text("Bu saatte tüm kategoriler yeniden taranır.") },
                    singleLine = true,
                    modifier = Modifier.fillMaxWidth()
                )
            }
